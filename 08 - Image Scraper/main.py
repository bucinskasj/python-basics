...
def get_high_res_img_url(img_node):
    ...
    srcsetlist = srcset.split(', ')

    url_res = [src.split(' ') for src in srcsetlist if img_filter(
        src, ['plus', 'profile', 'premium'])]
...
def save_images(img_urls, dest_dir="images", tag=""):
    for url in img_urls:
        ...
        with open(f"{dest_dir}/{tag}{file_name}.jpeg", "wb") as f:
            ...
            logging.info(
                f"Saved {file_name}, with {round(len(resp.content)/1024/1024, 2)} MB")
...
if __name__ == '__main__':
    ...
    # relevant_urls = [i for i in img_urls if img_filter(
