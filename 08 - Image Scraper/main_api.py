...
def get_response_for(keyword, results_per_page, page=1):
    url = f'https://unsplash.com/napi/search/photos?per_page={results_per_page}&query={keyword}&page={page}'
    resp = get(url)
...
def download_images(img_urls, max_downloads, dest_dir, tag=""):
    ...
    for url in img_urls:
        if successfully_downloaded < max_downloads:
            ...
            if not os.path.exists(f"images/{dest_dir}"):
                os.makedirs(f"images/{dest_dir}")

            with open(f"images/{dest_dir}/{tag}{file_name}.jpeg", 'wb') as f:
    ...
    while success_count < num_of_results:
        ...
        if (data):
            ...
            success_downloads = download_images(
                img_urls, max_downloads, dest_dir=keyword, tag=keyword)
